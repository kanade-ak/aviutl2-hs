...
import AviUtl2.Edit (PROJECT_FILE)
...
data OUTPUT_PLUGIN_TABLE = OUTPUT_PLUGIN_TABLE
  ...
  , optFuncLoadProjectConfig :: FunPtr (Ptr PROJECT_FILE -> IO BOOL_)
  , optFuncSaveProjectConfig :: FunPtr (Ptr PROJECT_FILE -> IO BOOL_)
  } deriving (Show)
...
outputFlagImage = 4
-- | プロジェクトファイル側に出力設定を保持できることを示すフラグです。
outputFlagProjectConfig :: CInt
outputFlagProjectConfig = 8
...
instance Storable OUTPUT_PLUGIN_TABLE where
  ...
  poke ptr v = do
    ...
    pokeByteOff ptr 56 (optFuncLoadProjectConfig v)
    pokeByteOff ptr 64 (optFuncSaveProjectConfig v)
