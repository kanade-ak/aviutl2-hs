...
module AviUtl2.Types
  ...
  , PALETTE_INFO(..)
  , EVENT_TYPE
  , eventTypeUpdateObject
  , eventTypeChangeEditFrame
  , eventTypeChangeEditScene
  , MODULE_TYPE
...
instance Storable TRACK_INFO where
  sizeOf _ = 24
  alignment _ = 8
  peek ptr = TRACK_INFO
    <$> peekByteOff ptr 0
    ...
    <*> peekByteOff ptr 23
  poke ptr v = do
    pokeByteOff ptr 0 (tiMode v)
    pokeByteOff ptr 8 (tiParam v)
    pokeByteOff ptr 16 (tiParamNum v)
    pokeByteOff ptr 20 (tiAccelerate v)
    pokeByteOff ptr 21 (tiDecelerate v)
    pokeByteOff ptr 22 (tiTwopoint v)
    pokeByteOff ptr 23 (tiTimecontrol v)

-- | パレット1色分のRGBA値です。SDK上ではアルファ値は通常255です。
data PALETTE_COLOR = PALETTE_COLOR
  { pcR :: Word8
  , pcG :: Word8
  , pcB :: Word8
  , pcA :: Word8
...
instance Storable PALETTE_COLOR where
  sizeOf _ = 4
  alignment _ = 1
  peek ptr = PALETTE_COLOR
    <$> peekByteOff ptr 0
    <*> peekByteOff ptr 1
    <*> peekByteOff ptr 2
    <*> peekByteOff ptr 3
  poke ptr v = do
    pokeByteOff ptr 0 (pcR v)
    pokeByteOff ptr 1 (pcG v)
    pokeByteOff ptr 2 (pcB v)
    pokeByteOff ptr 3 (pcA v)

-- | 64色分のパレット情報です。
newtype PALETTE_INFO = PALETTE_INFO
  { piColors :: [PALETTE_COLOR]
  } deriving (Show, Eq, Generic)

instance Storable PALETTE_INFO where
  sizeOf _ = 64 * sizeOf (undefined :: PALETTE_COLOR)
  alignment _ = alignment (undefined :: PALETTE_COLOR)
  peek ptr = PALETTE_INFO <$> forM [0 .. 63] (\i -> peekByteOff ptr (i * 4))
  poke ptr (PALETTE_INFO colors) =
    zipWithM_ (\i color -> pokeByteOff ptr (i * 4) color) [0 .. 63] padded
    where
      padded = take 64 (colors ++ repeat (PALETTE_COLOR 0 0 0 255))

-- | ホストイベント種別です。
type EVENT_TYPE = CInt

eventTypeUpdateObject :: EVENT_TYPE
eventTypeUpdateObject = 1

eventTypeChangeEditFrame :: EVENT_TYPE
eventTypeChangeEditFrame = 2

eventTypeChangeEditScene :: EVENT_TYPE
eventTypeChangeEditScene = 3
...
instance Storable OBJECT_INFO where
  ...
  poke ptr v = do
    ...
    pokeByteOff ptr 76 (oiLayer v)
    pokeByteOff ptr 80 (oiIndex v)
    pokeByteOff ptr 84 (oiNum v)
    pokeByteOff ptr 88 (oiFrameStart v)
    pokeByteOff ptr 92 (oiFrameEnd v)
...
-- | オブジェクト制御エフェクトを表す種別値です。
effectTypeControl :: EFFECT_TYPE
effectTypeControl = 4
-- | メディア出力エフェクトを表す種別値です。
effectTypeOutput :: EFFECT_TYPE
effectTypeOutput = 5
...
effectFlagFilter = 4
-- | カメラ効果を扱うエフェクトであることを示すフラグです。
effectFlagCamera :: EFFECT_FLAG
effectFlagCamera = 8
