...
itemTypeTrack = unsafePerformIO (newWideString "track2")
...
volumeItemPtr = unsafePerformIO $ do
  ptr <- malloc
  poke ptr (FILTER_ITEM_TRACK itemTypeTrack itemNameVolume 0.8 0.0 1.0 0.01 nullPtr 1.0)
  pure ptr
{-# NOINLINE volumeItemPtr #-}

frequencyAItemPtr = unsafePerformIO $ do
  ptr <- malloc
  poke ptr (FILTER_ITEM_TRACK itemTypeTrack itemNameFrequencyA 1000.0 200.0 2000.0 1.0 nullPtr 1.0)
  pure ptr
{-# NOINLINE frequencyAItemPtr #-}

frequencyBItemPtr = unsafePerformIO $ do
  ptr <- malloc
  poke ptr (FILTER_ITEM_TRACK itemTypeTrack itemNameFrequencyB 800.0 200.0 2000.0 1.0 nullPtr 1.0)
  pure ptr
{-# NOINLINE frequencyBItemPtr #-}

clickMsItemPtr = unsafePerformIO $ do
  ptr <- malloc
  poke ptr (FILTER_ITEM_TRACK itemTypeTrack itemNameClickMs 30.0 5.0 200.0 1.0 nullPtr 1.0)
  pure ptr
